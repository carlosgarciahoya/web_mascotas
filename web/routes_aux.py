...
def normalizar_ruta_foto(ruta: str | None) -> str | None:
    """
    Normaliza una ruta almacenada en la base de datos para que siempre
    sea relativa a 'static/' y use barras directas.

    - Convierte las barras invertidas \ en /
    - Si incluye 'static/', recorta desde ahí
    - Limpia barras iniciales duplicadas
    """
    if not ruta:
        return None
...
def crear_mascota():
    ...
    if request.method == "POST":
        ...
        for idx, f in enumerate(fotos):
            ...
            ruta_relativa = normalizar_ruta_foto(os.path.join("fotos", nombre_seguro))
        ...
        if not mascotas:
            ...
            for mascota in mascotas:
                ...
                for foto in fotos:
                    ...
                    if ruta_normalizada:
                        foto_url = url_for("static", filename=ruta_normalizada).lstrip("/")
                    else:
                        foto_url = ""
                    fotos_info.append({
...
@main.route("/modificar_mascotas")
def modificar_mascotas():
    mascotas = Mascota.query.all()
    return render_template("modificar.html", mascotas=mascotas)
