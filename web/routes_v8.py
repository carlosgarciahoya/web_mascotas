...
from sqlalchemy import or_
    # Nota: IntegrityError se mantiene por si queda algún constraint en BD
...
def _get_static_root() -> str:
    """
    Devuelve la carpeta estática efectiva desde la que Flask sirve los archivos.
    Si no hay contexto de aplicación, recurre a STATIC_DIR (por compatibilidad).
    """
    try:
...
def _resolver_ruta_absoluta(ruta_rel: str) -> str:
    """
    Construye un path absoluto seguro bajo la carpeta estática efectiva.
    """
...
def _es_mascota_desaparecida_valida(mascota: Mascota | None) -> bool:
    """
    Comprueba que la mascota desaparecida cumple los requisitos para iniciar la comparación.
    """
...
def _serializar_foto(foto: Foto) -> Dict[str, str] | None:
    """
    Devuelve la información necesaria de una foto (si el archivo existe físicamente).
    """
    ruta_rel = normalizar_ruta_foto(foto.ruta)
...
def _obtener_fotos_validas(mascota: Mascota | None) -> List[Dict[str, str]]:
    """
    Serializa y filtra las fotos de una mascota dejando sólo las que existen en disco.
    """
    if not mascota:
        return []
...
def _agrupar_fotos_por_tipo(fotos: List[Dict[str, str]]) -> Dict[str, List[Dict[str, str]]]:
    """
    Agrupa una lista de fotos serializadas por tipo de foto.
    """
...
def _comparar_imagenes_openai(prompt: str, data_urls: List[str]) -> str:
    """
    Realiza la llamada a OpenAI para comparar o describir imágenes.
    """
...
def _programar_envio_correo(mascota_id: int) -> None:
    ...
    try:
        ...
        # Cargar siempre las variables SMTP desde el entorno en la app
...
def crear_mascota(mascota_id=None):
    ...
    if request.method == "POST":
        ...
        # ===== Control de duplicados (cualquier tipo): tipo, zona, especie, color, tamaño =====
        ...
        if dup_q.first():
            flash("Ya existe un registro con el mismo tipo, zona, especie, color y tamaño.", "error")
            return redirect(request.url)
        # ======================================================================================
...
# =============================
# NUEVO FLUJO: COMPARAR MASCOTAS
# =============================

@main.route("/comparar_mascotas/desaparecidas", methods=["GET", "POST"])
def comparar_mascotas_desaparecidas():
    """
    Paso 1 del flujo: seleccionar una mascota desaparecida válida para iniciar las comparaciones.
    """
    busqueda_realizada = request.method == "POST"
    mensaje = None
...
def comparar_mascotas_candidatas(desaparecida_id: int):
    """
    Paso 2: listar las mascotas encontradas candidatas para comparar con la desaparecida elegida.
    """
    ...
    # Validación adicional: la desaparecida debe tener fotos
...
def comparar_mascotas_detalle(desaparecida_id: int, encontrada_id: int):
    """
    Paso 3: enfrentamiento entre una desaparecida y una encontrada.
    Permite comparar fotos de tipos iguales o comparar todas las fotos disponibles.
    """
...
# =============================
# COMPARAR FOTOS (FORMULARIO GENÉRICO EXISTENTE)
# =============================
...
def comparar_fotos():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Limpiamos los archivos temporales creados para comparar
            for ruta in rutas:
